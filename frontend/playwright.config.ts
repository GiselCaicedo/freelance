...
const experiences = ['shared', 'admin', 'client'] as const;
...
export default defineConfig<ChromaticConfig>({
  testDir: './tests/e2e',
  // Look for files with the .spec.js or .e2e.js extension inside the experience folders
  ...
  projects: experiences.flatMap((experience) => [
    {
      name: `${experience}-chromium`,
      testMatch: `${experience}/**/*.@(spec|e2e).?(c|m)[jt]s?(x)`,
      use: { ...devices['Desktop Chrome'] },
    },
    ...(process.env.CI
      ? [
          {
            name: `${experience}-firefox`,
            testMatch: `${experience}/**/*.@(spec|e2e).?(c|m)[jt]s?(x)`,
