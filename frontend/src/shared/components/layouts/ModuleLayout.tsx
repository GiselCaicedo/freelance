...
export default function ModuleLayout({ variant, children }: ModuleLayoutProps) {
  ...
  const NavLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      <Link
        href={href}
        className={`group flex items-center gap-2 px-3 py-2.5 rounded-lg ${TRANSITION_ALL} relative ${
          active ? 'bg-emerald-50 text-emerald-700 font-medium' : 'text-gray-600 hover:bg-gray-50'
        }`}
        title={!isNavOpen ? item.label : ''}
      >
        <item.icon
          className={`w-5 h-5 ${TRANSITION_ALL} flex-shrink-0 ${
        ...
        <div className={`flex h-screen flex-col bg-white transition-all duration-300 ${isNavOpen ? 'w-52' : 'w-16'}`}>
          <div className={`flex ${isNavOpen ? 'justify-end px-2 pt-2' : 'justify-center pt-2'}`}>
            <button
              ...
              className={`p-1.5 rounded-md text-gray-600 ${TRANSITION_COLORS} hover:bg-gray-100 hover:text-gray-900`}
