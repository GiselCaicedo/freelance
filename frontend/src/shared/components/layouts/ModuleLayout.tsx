...
export default function ModuleLayout({ variant, children }: ModuleLayoutProps) {
  ...
  const navItems = useMemo<NavItem[]>(
    () => [
      ...
      { slug: 'invoices', label: t('nav.invoices'), icon: FileText, permission: 'admin' },
  ...
  const NavLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      <Link
        href={href}
        className={`group flex items-center gap-2 rounded-lg px-3 py-2.5 ${TRANSITION_ALL} relative ${
          active ? 'bg-sky-50 font-medium text-sky-700' : 'text-gray-600 hover:bg-gray-50'
        }`}
        title={!isNavOpen ? item.label : ''}
      >
        <item.icon
          className={`h-5 w-5 ${TRANSITION_ALL} flex-shrink-0 ${
            active ? 'text-sky-500' : 'text-gray-400 group-hover:text-sky-500'
          }`}
        />
        {isNavOpen && (
          <>
          ...
          <div className="flex justify-center border-b border-gray-200 px-3 py-5">
            {isNavOpen && (
              <div className="flex min-w-0 gap-2">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-sky-500">
          ...
          {/* {isNavOpen && (
            <div className="px-3 py-3">
              <button
                ...
                className={`flex w-full items-center justify-between gap-2 rounded-lg border px-2.5 py-2 text-sm ${TRANSITION_ALL} ${
                  isEnterpriseOpen ? 'border-sky-300 bg-sky-50' : 'border-gray-200 bg-gray-50 hover:border-gray-300 hover:bg-white'
                }`}
