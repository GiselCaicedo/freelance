...
import { detectCategoryFromPermissionName } from './panel.utils.js'
...
export async function replaceRolePermissionsSvc(roleId: string, permissionIds: string[]) {
  return prisma.$transaction(async (tx) => {
    ...
    const panelFromIncoming = permissions
      .map((permission) => ({ id: permission.id, category: detectCategoryFromPermissionName(permission.name) }))
      .filter((item): item is { id: string; category: 'admin' | 'client' } => Boolean(item.category))
