...
function buildPeriod(from?: string, to?: string): DashboardPeriod {
  ...
  const periodTo = parsedTo ?? (parsedFrom ? nextMonthStart(parsedFrom) : defaultTo)
...
export async function getDashboardSummaryCtrl(req: Request, res: Response) {
  try {
    ...
    const referenceDate = parseDate(req.query.referenceDate) ?? period.to
...
export async function getUpcomingExpirationsCtrl(req: Request, res: Response) {
  try {
    ...
    const expirations = await fetchUpcomingExpirations(referenceDate, monthsAhead)
    res.json({
      success: true,
      data: {
